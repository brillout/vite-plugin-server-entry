...
type ConfigUnresolved = ConfigVite & {
  ...
  _vitePluginImportBuildResolved?: PluginConfigResolved
}
...
function resolveConfig(
  ...
  const pluginConfigResolved: PluginConfigResolved = configUnresolved._vitePluginImportBuildResolved ?? {
    libraries: [],
...
async function generateImportBuildFile(emitFile: EmitFile, rollupBundle: RollupBundle, config: ConfigResolved) {
  ...
  if (config._vitePluginImportBuildResolved.importerAlreadyGenerated) return
  config._vitePluginImportBuildResolved.importerAlreadyGenerated = true
  ...
  const source = [
    ...
    ...config._vitePluginImportBuildResolved.libraries.map(({ getImporterCode }) =>
      getImporterCode({
...
function autoImporterIsDisabled(config: ConfigResolved): boolean {
  return config._vitePluginImportBuildResolved.disableAutoImporter ?? isYarnPnP()
}

function isUsingOlderVitePluginImportBuildVersion(config: ConfigResolved): boolean {
  return config._vitePluginImportBuildResolved.libraries.some((library) => {
